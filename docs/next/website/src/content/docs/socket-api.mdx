...
`PaneInfo` and `AgentInfo` expose `background_work: true` when manifest detection finds outstanding background work. The field is omitted when false. It is orthogonal to `agent_status`, so idle, working, and blocked agents can all report background work without changing status. The UI presents idle plus background work as `waiting`. A background-only edge increments the pane `revision` and emits `pane.updated`; it does not normally emit `pane.agent_status_changed` or advance `state_change_seq`. When completion was deferred, the debounced falling edge delivers the owed completion exactly once, advances `state_change_seq`, increments `revision`, and emits `pane.updated`. It emits `pane.agent_status_changed` only when the status or presentation actually changes, so clients should use `pane.updated` and compare `revision` or `state_change_seq` as the reliable falling-edge signal.
...
The response contains the same explain object printed by `herdr agent explain --json`, including the final state, background-work signal and contributing rules, manifest source and version, matched rule, evaluated rule evidence, skip-state reason, idle fallback reason, and `screen_detection_skip_reason` when a full lifecycle hook authority makes screen rules non-authoritative.
