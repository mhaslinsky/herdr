...
`PaneInfo` and `AgentInfo` expose `background_work: true` when manifest detection finds outstanding background work. The field is omitted when false. It is orthogonal to `agent_status`, so idle, working, and blocked agents can all report background work without changing status. A background-only edge increments the pane `revision` and emits `pane.updated`; it does not emit `pane.agent_status_changed` or advance `state_change_seq`.
