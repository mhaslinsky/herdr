...
`PaneInfo.terminal_generation` increments whenever Herdr replaces the runtime
behind a terminal id. Automation that submits work for later processing should
bind it to both values so a replacement process cannot inherit stale work.
...
`PaneInfo.wait_lease` is present while a wait lease is active and contains its
job id and remaining milliseconds. Acquire and release use the
`herdr pane wait-lease` CLI's atomic-file transport rather than socket methods.
Treat a lease only as evidence for suppressing a false stall alarm. It does not
prove job health and must never authorize resource teardown or termination.

`PaneInfo.background_work` is `true` while the pane has outstanding background
work — an agent's background shell detected on screen, or an active wait lease —
even after the main thread has returned to the prompt. It is orthogonal to
`agent_status`: a pane can be idle, working, or blocked with `background_work`
set, and the field is omitted when `false`. A client should render an idle pane
that has `background_work` as "waiting" (chattable, but not finished) rather than
as done. It never changes `agent_status`, so it cannot mask a blocked pane.
