...
| `waiting` | The agent is ready for input while detected background work is still running. |
| `done` | The agent finished and you have not looked at it yet. |
| `idle` | The agent is ready for input, has no detected background work, and has been seen. |
