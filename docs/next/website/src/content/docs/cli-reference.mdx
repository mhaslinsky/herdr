...
Declare a bounded background wait without changing the pane's detected agent status:

```bash
herdr pane wait-lease acquire [pane_id] --id <job-ref> --ttl-ms N
herdr pane wait-lease release [pane_id] --id <job-ref> --token <token>
```

Omit `pane_id` inside a Herdr pane to use `HERDR_PANE_ID`. Acquire returns an opaque owner token. Save it only long enough to release the lease; a release with a different job id or token is rejected. The TTL is required and capped at 24 hours. Requests use atomic, owner-private files in a server-owned transport directory beside the active socket. The server clears that directory when it starts and acknowledges an acquire only after applying it; if the acknowledgement cannot be written, it rolls the acquire back. `pane get` and `pane list` expose an active lease as `wait_lease` with its `job_id` and remaining time, while the navigator shows an `awaiting` annotation next to the unchanged detected status. Leases are bound to the pane's current terminal runtime and disappear when that runtime is replaced, when the server restarts, or when the lease expires.

A lease can suppress a false stall alarm, but it must never authorize teardown or resource termination. Require independent job evidence and an operator policy time floor before any destructive action.
