...
Omit `pane_id` inside a Herdr pane to use `HERDR_PANE_ID`. Acquire returns an opaque owner token. Save it only long enough to release the lease; a release with a different job id or token is rejected. The TTL is required and capped at 24 hours. Requests use atomic files under the active Herdr runtime directory and are acknowledged only after the server applies them. `pane get` and `pane list` expose an active lease as `wait_lease` with its `job_id` and remaining time, while the navigator shows an `awaiting` annotation next to the unchanged detected status. Leases are bound to the pane's current terminal generation and disappear on server restart or expiry.
