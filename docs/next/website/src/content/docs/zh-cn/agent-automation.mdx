...
`agent prompt --wait` 会立即提交提示。智能体从非 working 状态开始时，Herdr 首先要求在五秒内观察到 working 或 blocked 生命周期活动。如果没有观察到活动，它会返回 `agent_prompt_stalled`，而不是无限等待；调用方设置的 `--timeout` 不超过五秒时，仍返回普通的 `timeout` 错误。观察到活动后，它会等待请求的稳定状态。它不会跟踪单独的轮次。如果智能体已经处于 working，当前轮次的完成可能满足等待。独立的 `agent wait` 会观察当前智能体；如果状态已经匹配，就会立即返回。两者默认匹配 `idle`、`done` 或 `blocked`。可以重复使用 `--until` 接受多个精确状态，例如 `--until idle --until done`；需要 `unknown` 时请明确使用 `--until unknown`。当 `background_work` 为 true 时，`idle` 和 `done` 都不会满足这两个命令，即使显式请求也是如此；`blocked` 仍会满足。在 `agent prompt` 中，`--until` 必须与 `--wait` 一起使用。

`idle` 表示智能体已可接受输入，没有检测到后台工作，且其标签页已在聚焦的 Herdr 界面中显示。idle 智能体仍有检测到的后台工作时，UI 会将其标为 `waiting`，完成通知也保持待处理。`done` 是后台工作结束后的相同底层 idle 状态,直到该标签页获得焦点或 `pane focus` / `agent focus` 指向它。仅通过 CLI 读取不会把它标记为已查看。`blocked` 表示 Herdr 识别到了审批或提问界面。`unknown` 表示智能体存在,但 Herdr 无法可靠判断其生命周期;它不代表工作成功完成。区别重要时,请指定精确的 `--until` 状态。
