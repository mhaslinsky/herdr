...
A blocked agent makes its pane, tab, and workspace look blocked. A working agent makes the workspace look active. An idle agent with detected background work appears as blue `waiting`, remains promptable, and does not become done until that work clears. A done agent stays visible until you view it.
