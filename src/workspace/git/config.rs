    ...
    fn read(&mut self, path: &Path) -> (PathBuf, Option<String>) {
        ...
        let contents = self
            ...
            .or_insert_with(|| {
                ...
                dep.2 &= read_result.is_ok()
                    || matches!(&read_result, Err(error) if error.kind() == ErrorKind::NotFound);
                deps.push(dep);
                read_result.ok()
            })
            .clone();
