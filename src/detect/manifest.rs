...
struct ManifestRule {
    ...
    /// Marks the pane as having outstanding background work. Orthogonal to
    /// `state`: a rule may set this with `state` omitted so it contributes the
    /// background-work signal without competing in state arbitration. Gated
    /// behind `min_engine_version` >= BACKGROUND_WORK_ENGINE_VERSION.
    #[serde(default)]
...
fn evaluate_loaded_manifest(
    ...
    for (rule, compiled_rule) in loaded.manifest.rules.iter().zip(&loaded.compiled_rules) {
        ...
        // Background work is an orthogonal signal: OR it in from every matched rule, regardless of
        // priority or which rule wins state arbitration below.
        if rule.background_work {
            background_work = true;
        }

        // A rule without a `state` (a background-work-only signal) does not compete in state
        // arbitration; skipping it here keeps it from winning and forcing Unknown.
