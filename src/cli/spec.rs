    ...
    Command::new("agent")
        ...
        .subcommand(
            Command::new("prompt")
                ...
                .after_help(
                    "If the agent is already blocked, submission is rejected with agent_blocked before any input is sent. When an accepted submission starts from another non-working state, --wait first requires an observed state change within 5000ms; otherwise it returns agent_prompt_stalled. A shorter --timeout returns timeout instead. It then matches idle, done, or blocked by default, or any exact --until state. Detected background work prevents idle and done from settling, even when requested explicitly. It does not track turns: if the agent is already working, that active turn's completion may match. Without --timeout, the settled-state wait is indefinite.",
                ),
        )
        .subcommand(
            Command::new("rename")
