...
// Threads the caller's per-pane "last published" detection trackers by mutable ref so the loop's
// dedup state stays in one place; adding background_work pushed this one over the arg lint.
...
async fn apply_agent_detection_publish_update(
    ...
    *last_background_work = update.background_work;
